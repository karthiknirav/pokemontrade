...
export default async function ProductDetailPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const liveListings = getActionableListings(product.listingSnapshots);
  const recentSales = getLastThreeSales(product.salesRecords);
  ...
  return (
    <AppShell title={product.name} subtitle={product.notes ?? "Product detail and profit outlook for the Australian market."}>
      <div className="grid gap-6 xl:grid-cols-[0.9fr_1.1fr]">
        <section className="rounded-3xl border border-mist bg-white p-5 shadow-sm">
          <div className="flex items-start justify-between gap-4">
            <div>
              <div className="text-sm text-slate-500">{product.set?.name}</div>
              <div className="mt-2 text-3xl font-semibold">{formatAud(Number(product.currentMarketPrice))}</div>
              <div className="mt-1 text-sm text-slate-500">Last sold {formatAud(Number(product.lastSoldPrice))}</div>
            </div>
          ...
          <div className="mt-5 rounded-2xl bg-slate-50 p-4">
            <div className="text-sm text-slate-500">Plain-English recommendation</div>
            <div className="mt-2 font-medium text-ink">{guardedRecommendation.summary}</div>
        ...
        <section className="rounded-3xl border border-mist bg-white p-5 shadow-sm">
          ...
          <div className="mt-4 grid gap-3 md:grid-cols-3">
            <div className="rounded-2xl bg-slate-50 p-4">
              <div className="text-sm text-slate-500">3 months</div>
              <div className="mt-1 font-semibold">{formatAud(Number(snapshot?.estimated3m ?? 0))}</div>
            </div>
            <div className="rounded-2xl bg-slate-50 p-4">
              <div className="text-sm text-slate-500">1 year</div>
              <div className="mt-1 font-semibold">{formatAud(Number(snapshot?.estimated1y ?? 0))}</div>
            </div>
            <div className="rounded-2xl bg-slate-50 p-4">
              <div className="text-sm text-slate-500">3 years</div>
              <div className="mt-1 font-semibold">{formatAud(Number(snapshot?.estimated3y ?? 0))}</div>
            </div>
          </div>
        </section>
      </div>
